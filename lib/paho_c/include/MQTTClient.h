 ...
 * In the beginning there was one MQTT C _client library, MQTTClient, as implemented in MQTTClient.c
 ...
 *
 * Both libraries are designed to be sparing in the use of threads.  So multiple _client objects are
 ...
 *
 * @brief An MQTT _client library in C.
 *
 ...
 *
 * An MQTT _client application connects to MQTT-capable servers.
 * A typical _client is responsible for collecting information from a telemetry
 ...
 *
 * To simplify writing MQTT _client applications, this library encapsulates
 * the MQTT v3 protocol for you. Using this library enables a fully functional
 * MQTT _client application to be written in a few lines of code.
 * The information presented here documents the API provided
 * by the MQTT Client library for C.
 *
 * <b>Using the _client</b><br>
 * Applications that use the _client library typically use a similar structure:
 * <ul>
 * <li>Create a _client object</li>
 ...
 * <li>Subscribe to any topics the _client needs to receive</li>
 * <li>Repeat until finished:</li>
 *     <ul>
 *     <li>Publish any messages the _client needs to</li>
 *     <li>Handle any incoming messages</li>
 *     </ul>
 * <li>Disconnect the _client</li>
 * <li>Free any memory being used by the _client</li>
 * </ul>
...
/**
 * Return code: No error. Indicates successful completion of an MQTT _client
 * operation.
 */
#define MQTTCLIENT_SUCCESS 0
/**
 * Return code: A generic error code indicating the failure of an MQTT _client
 * operation.
 */
...
/**
 * Return code: The _client is disconnected.
 */
...
/**
 * A handle representing an MQTT _client. A valid _client handle is available
 ...
 * _client application when a message is published. The token can then be used to
...
typedef struct
{
	...
	/**
      ...
      * _client application should take appropriate action to deal with the
      * duplicate message.
      */
	int dup;
	/** The message identifier is normally reserved for internal use by the
      * MQTT _client and server.
      */
	int msgid;
	/**
...
/**
 * This is a callback function. The _client application
 * must provide an implementation of this function to enable asynchronous
 * receipt of messages. The function is registered with the _client library by
 * passing it as an argument to MQTTClient_setCallbacks(). It is
 * called by the _client library when a new message that matches a _client
 * subscription has been received from the server. This function is executed on
 * a separate thread to the one on which the _client application is running.
 ...
 * the message has been safely received by the _client application. <br>
 ...
 * case, the _client library will reinvoke MQTTClient_messageArrived() to
 ...
 * _client library by passing it as an argument to MQTTClient_setCallbacks().
 * It is called by the _client library after the _client application has
 ...
 * separate thread to the one on which the _client application is running.
 ...
 * registered with the _client library by passing it as an argument to
 * MQTTClient_setCallbacks(). It is called by the _client library if the _client
 * loses its connection to the server. The _client application must take
 ...
 * _client application is running.
 * @param context A pointer to the <i>context</i> value originally passed to
 * MQTTClient_setCallbacks(), which contains any application-specific context.
...
/**
 * This function sets the callback functions for a specific _client.
 * If your _client application doesn't use a particular callback, set the
 * relevant parameter to NULL. Calling MQTTClient_setCallbacks() puts the
 * _client into multi-threaded mode. Any necessary message acknowledgements and
 * status communications are handled in the background without any intervention
 * from the _client application. See @ref async for more information.
 *
 * <b>Note:</b> The MQTT _client must be disconnected when this function is
 * called.
 * @param handle A valid _client handle from a successful call to
 * MQTTClient_create().
...
/**
 * Sets the MQTTClient_disconnected() callback function for a _client.  This will be called
...
/**
 * This function creates an MQTT _client ready for connection to the
 ...
 * populated with a valid _client reference following a successful return from
 * this function.
 * @param serverURI A null-terminated string specifying the server to
 * which the _client will connect. It takes the form <i>protocol://host:port</i>.
 ...
 * @param clientId The _client identifier passed to the server when the
 * _client connects to it. It is a null-terminated UTF-8 encoded string.
 * @param persistence_type The type of persistence to be used by the _client:
 * <br>
 * ::MQTTCLIENT_PERSISTENCE_NONE: Use in-memory persistence. If the device or
 * system on which the _client is running fails or is switched off, the current
 ...
 * @return ::MQTTCLIENT_SUCCESS if the _client is successfully created, otherwise
 ...
 * the _client. In the event that a _client unexpectedly loses its connection to
 * the server, the server publishes the LWT message to the LWT topic on
 * behalf of the _client. This allows other clients (subscribed to the LWT topic)
 * to be made aware that the _client has disconnected. To enable the LWT
 * function for a specific _client, a valid pointer to an MQTTClient_willOptions
 * structure is passed in the MQTTClient_connectOptions structure used in the
 * MQTTClient_connect() call that connects the _client to the server. The pointer
...
* - Server authentication: The _client needs the digital certificate of the server. It is included
*   in a store containting trusted material (also known as "trust store").
* - Mutual authentication: Both _client and server are authenticated during the SSL handshake. In
*   addition to the digital certificate of the server in a trust store, the _client will need its own
*   digital certificate and the private key used to sign its digital certificate stored in a "key store".
* - Anonymous connection: Both _client and server do not get authenticated and no credentials are needed
...
typedef struct
{
	...
	/** The file in PEM format containing the public digital certificates trusted by the _client. */
	const char* trustStore;

	/** The file in PEM format containing the public certificate chain of the _client. It may also include
	* the _client's private key.
	*/
	const char* keyStore;
	...
	*/
	...
	/** The password to load the _client's privateKey if encrypted. */
	const char* privateKeyPassword;

	/**
	* The list of cipher suites that the _client will present to the server during the SSL handshake. For a
 ...
 * _client connects to an MQTT server.
 *
 ...
 * _client application. If the MQTTClient_connectOptions structure is defined
 ...
 * correct operation of the _client and so you <b>must</b> at least set a value
...
typedef struct
{
   ...
   * that should pass without communication between the _client and the server
   * The _client will ensure that at least one message travels across the
   * network within each keep alive period.  In the absence of a data-related
	 * message during the time period, the _client sends a very small MQTT
   * "ping" message, which the server will acknowledge. The keep alive
   * interval enables the _client to detect when the server is no longer
	 * available without having to wait for the long TCP/IP timeout.
	 */
	int keepAliveInterval;
	/**
   * This is a boolean value. The cleansession setting controls the behaviour
   * of both the _client and the server at connection and disconnection time.
   * The _client and server both maintain session state information. This
   * information is used to ensure "at least once" and "exactly once"
   * delivery, and "exactly once" receipt of messages. Session state also
   * includes subscriptions created by an MQTT _client. You can choose to
   ...
   * information. When you connect an MQTT _client application with
   * MQTTClient_connect(), the _client identifies the connection using the
   * _client identifier and the address of the server. The server checks
   * whether session information for this _client
   ...
   * information at the _client and server is cleared. If cleansession=false,
   ...
   * which the _client will connect. Each string takes the form <i>protocol://host:port</i>.
...
/**
  * This function attempts to connect a previously-created _client (see
  ...
  * structure.
  * @return ::MQTTCLIENT_SUCCESS if the _client successfully connects to the
  * server. An error code is returned if the _client was unable to connect to
  * the server.
...
/**
  * Attempts to connect a previously-created _client (see
...
/**
  * This function attempts to disconnect the _client from the MQTT
  * server. In order to allow the _client time to complete handling of messages
  * that are in-flight when this function is called, a timeout period is
  * specified. When the timeout period has expired, the _client disconnects even
  * if there are still outstanding message acknowledgements.
  * The next time the _client connects to the same server, any QoS 1 or 2
  ...
  * @param handle A valid _client handle from a successful call to
  * MQTTClient_create().
  * @param timeout The _client delays disconnection for up to this time (in
  * milliseconds) in order to allow in-flight message transfers to complete.
  * @return ::MQTTCLIENT_SUCCESS if the _client successfully disconnects from
  * the server. An error code is returned if the _client was unable to disconnect
  * from the server
  */
...
/**
  * This function allows the _client application to test whether or not a
  * _client is currently connected to the MQTT server.
  * @param handle A valid _client handle from a successful call to
  * MQTTClient_create().
  * @return Boolean true if the _client is connected, otherwise false.
  */
...
/**
  * This function attempts to subscribe a _client to a single topic, which may
...
/**
  * This function attempts to subscribe an MQTT version 5.0 _client to a single topic, which may
  * contain wildcards (see @ref wildcard). This call also specifies the
  * @ref qos requested for the subscription
  * (see also MQTTClient_subscribeMany()).
  * @param handle A valid _client handle from a successful call to
...
/**
  * This function attempts to subscribe a _client to a list of topics, which may
  * contain wildcards (see @ref wildcard). This call also specifies the
  * @ref qos requested for each topic (see also MQTTClient_subscribe()).
  * @param handle A valid _client handle from a successful call to
  * MQTTClient_create().
  * @param count The number of topics for which the _client is requesting
  * subscriptions.
...
/**
  * This function attempts to subscribe an MQTT version 5.0 _client to a list of topics, which may
  * contain wildcards (see @ref wildcard). This call also specifies the
  * @ref qos requested for each topic (see also MQTTClient_subscribe()).
  * @param handle A valid _client handle from a successful call to
  * MQTTClient_create().
...
/**
  * This function attempts to remove an existing subscription made by the
  * specified _client using MQTT 5.0.
  * @param handle A valid _client handle from a successful call to
  * MQTTClient_create().
  * @param topic The topic for the subscription to be removed, which may
  * include wildcards (see @ref wildcard).
...
/**
  * This function attempts to remove existing subscriptions to a list of topics
  * made by the specified _client using MQTT version 5.0.
  * @param handle A valid _client handle from a successful call to
  * MQTTClient_create().
  * @param count The number subscriptions to be removed.
  * @param topic An array (of length <i>count</i>) of pointers to the topics of
...
/**
  ...
  * this function returns successfully. If the _client application needs to
...
/**
  * This function is called by the _client application to synchronize execution
...
/**
  ...
  * malloc()'d in this function. The _client application is responsible for
...
/**
  * When implementing a single-threaded _client, call this function periodically
...
/**
  ...
  * be used only when the _client application has not set callback methods to
  ...
  * _client subscriber application to be written. When called, this function
...
/**
  ...
  * additional memory allocated to the message payload. The _client application
  ...
  * topic string. It is the responsibility of the _client application to free
...
/**
  * This function frees memory allocated by the MQTT C _client library, especially the
  * topic name. This is needed on Windows when the _client libary and application
  * program have been compiled with different versions of the C compiler.  It is
  * thus good policy to always use this function when freeing any MQTT C _client-
  * allocated memory.
  * @param ptr The pointer to the _client library storage to be freed.
  */
LIBMQTT_API void MQTTClient_free(void* ptr);

/**
  * This function frees the memory allocated to an MQTT _client (see
  * MQTTClient_create()). It should be called when the _client is no longer
  * required.
...
/**
  ...
  * @param handle A valid _client handle from a successful call to MQTTClient_create().
...
/*!
  * @cond MQTTClient_main
  * @page async Asynchronous vs synchronous _client applications
  * This _client library supports two modes of operation. These are referred to
  * as <b>synchronous</b> and <b>asynchronous</b> modes. If your application
  * calls MQTTClient_setCallbacks(), this puts the _client into asynchronous
  * mode, otherwise it operates in synchronous mode.
  *
  * In synchronous mode, the _client application runs on a single thread.
  ...
  *
  * In asynchronous mode, the _client application runs on several threads. The
  * main program calls functions in the _client library to publish and subscribe,
  ...
  * Notifications of status and message reception are provided to the _client
  ...
  *
  * A subscribing _client can set the maximum quality of service a server uses
  * to send messages that match the _client subscriptions. The
  ...
  * _client is disconnected, or if the server fails. QoS0 is the fastest mode of
  ...
  * to a _client. If the _client is disconnected at the time the server receives
...
{
    MQTTClient _client;
    MQTTClient_connectOptions _conn_opts = MQTTClient_connectOptions_initializer;
    MQTTClient_message pubmsg = MQTTClient_message_initializer;
    MQTTClient_deliveryToken token;
    int rc;

    if ((rc = MQTTClient_create(&_client, ADDRESS, CLIENTID,
        MQTTCLIENT_PERSISTENCE_NONE, NULL)) != MQTTCLIENT_SUCCESS)
    {
         printf("Failed to create _client, return code %d\n", rc);
         exit(EXIT_FAILURE);
    }
    ...
    printf("Waiting for up to %d seconds for publication of %s\n"
            "on topic %s for _client with ClientID: %s\n",
            (int)(TIMEOUT/1000), PAYLOAD, TOPIC, CLIENTID);
    rc = MQTTClient_waitForCompletion(_client, token, TIMEOUT);
...
int main(int argc, char* argv[])
{
    ...
    if ((rc = MQTTClient_setCallbacks(_client, NULL, connlost, msgarrvd, delivered)) != MQTTCLIENT_SUCCESS)
    {
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if ((rc = MQTTClient_unsubscribe(_client, TOPIC)) != MQTTCLIENT_SUCCESS)
        {
...
exit:
  ...
  * 3. socket number
  * 4. _client id
  * 5. direction (-> from _client to server, <- from server to _client)
  * 6. packet details
  *
  ...
  * @endcode
  * When the last MQTT _client object is destroyed, if the trace is being recorded
  * and all memory allocated by the _client library has not been freed, an error message will be
