 ...
 * the MQTT _client is created (see MQTTClient_create()). The default
...
/**
  ...
  * 
  * Either open the existing persistent store for this _client ID or create a new
  * one if one doesn't exist. If the persistent store is already open, return 
  * without taking any action.
  *
  * An application can use the same _client identifier to connect to many
  ...
  * @param clientID The _client identifier for which the persistent store should
  * be opened.
  * @param serverURI The connection string specified when the MQTT _client was
